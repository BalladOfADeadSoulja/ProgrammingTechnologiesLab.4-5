...
bool multiplication(HWND hWnd) {
	...
	HINSTANCE load;
	load = LoadLibrary(L"DynamicDLL.dll");
	typedef int (*mull) (int, int);
	mull Multiply;
	Multiply = (mull)GetProcAddress(load, "Mull");*/

	//if (hDll == NULL) {
	//	// библиотека не была загружена
	//	MessageBox(NULL, L"Библиотека Dynamic.dll не найдена", L"Ошибка", MB_OK | MB_ICONERROR);
	//	//return true;
	//}
